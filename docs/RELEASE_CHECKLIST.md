...
- [x] The committed Marketplace Evidence Index reports exact `sampled`, `expected`, and `dropped`
      counts for discovery query `a`; equality means a complete response for that query, while a
      mismatch renders partial/degraded without attributing a cause.
...
- [x] Attack Theater starts idle and sends no request before **Run test sequence** is selected;
      reduced-motion users advance subsequent cases explicitly.
...
- [ ] Verify long payloads, endpoint hosts, attestation IDs, reason codes, service names, and code blocks
      wrap or scroll.
- [ ] Verify `/theater` sends no request before explicit activation, then advances its counter only
      on accepted live verdict-and-receipt responses; errors or malformed receipts stay stopped and
      visible.
...
- [ ] Hosted CI passes on the exact release commit; local results are not presented as hosted-CI
      evidence.
...
- [x] The homepage leads with verifiable pre-action security, the action boundary, and inspectable
      evidence.
- [x] `/trust` keeps caller-side enforcement, signed APA evidence, public transparency, and dated
      marketplace or audit context as distinct layers.
...
- [x] Marketplace Evidence Index copy describes schema-v2 `sampled`/`expected`/`dropped`
      coverage and labels the atomic 30-minute timer source-ready but not deployed.
- [ ] Python and TypeScript packages are published to approved registries and their public install
      coordinates are verified. Source-built package checks do not satisfy this item.
...
above. Hosted CI, package publication, Nginx, systemd, deploy, and VPS verification also remain
unchecked. Do not convert those items to passed from source inspection or local automated tests
alone.

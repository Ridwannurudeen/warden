...
async def poll_once(
    ...
    *,
    ...
    """Answer every task in the queue once. Declines; never accepts.
    ...
    for decision in decide(config, fetch_provider_tasks(config)):
        if decision.action == "refuse" and runner.store.already_answered(decision.job_id):
            results.append(
        ...
        if effective == "refused":
            ...
            runner.store.mark_answered(decision.job_id, "refused", decision.reason)
        else:
            results.append(
...
async def _main() -> int:
    ...
    parser.add_argument(
        "--apply-decisions",
        action="store_true",
        help="actually decline the tasks the gate rejected (default: describe only)",
    )

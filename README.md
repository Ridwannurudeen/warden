<div align="center">
  ...
  <h1>Warden</h1>
  <p><strong>Deterministic payload firewall and pre-listing auditor for A2MCP agents.</strong></p>
  <p>
...
![Warden landing page](docs/screenshots/warden-landing-desktop.png)
...
The live service is `https://warden.gudman.xyz`. Production `/health` is public. Production `/scan` and `/audit` return HTTP 402 until the caller pays through the OKX x402 challenge.
...
- Run `python -m pytest -q` and `python -m ruff check .` after changes.

...
The following are not represented as fixed. The first three were reproduced against `b97ea75` and block any claim that the current badge is certification-grade.

| ID | Severity | Location | Reproduction and impact | Why deferred / required next step | Status |
|---|---|---|---|---|---|
| D-01 | High | `warden/badges.py:81-113`, `warden/marketplace/render.py:277-294` | Audits of two different paths on the same host produced the same signed identity, and a host badge associated with a service on a different path or port. Scheme, port, path, query, and battery version are absent from the signed record. | Add a versioned badge record that signs a canonical exact target and make marketplace association exact. That changes the frozen nested audit evidence contract and needs a migration policy for existing records. | DEFERRED |
| D-02 | High | `warden/auditor.py:60-180` | A target that recognizes and rejects all 20 public attack probes received a signed A/100 without any benign or liveness control. | Define a versioned, reviewable audit policy with benign controls, target-liveness semantics, battery identity, and resistance to simple fixed-probe recognition. Do not market legacy badges as general safety certification. | DEFERRED |
| D-03 | High | `warden/auditor.py:69-154` | A target blocking 16/20 fixed probes scored B/80, but 20 trivially blocked caller-supplied prompts raised the same target to signed A/90 (36/40). | Separate fixed-battery evidence from caller diagnostics and sign prompt provenance/battery hash. A narrow denominator change would make the response internally ambiguous because the frozen schema has no provenance fields. | DEFERRED |
| D-04 | Medium | `warden/auditor.py:352-438`, `deploy/DEPLOY.md:44` | Consent is soft by default; a paid caller can cause 20–40 active POST probes to a public target that did not opt in. SSRF controls, payment, and rate limits reduce but do not remove the abuse boundary. | Recommended production posture is hard consent before any probe. Changing the default requires an operator/product decision and deployment migration. | DEFERRED |
| D-05 | Medium | `warden/api.py:380-401`, `tests/test_ratelimit.py:109-124` | Mere presence of a forged payment header selects the 600/minute bucket instead of the normal 60/minute bucket before x402 verification. This does not bypass settlement, but increases facilitator-verification work an unauthenticated source can trigger. | Grant elevated throughput only after verified settlement, or cap pre-verification payment attempts at the ordinary limit. Middleware ordering and paid-client replay behavior need an integration design. | DEFERRED |
| D-06 | Medium | `warden/badge_store.py:17-45` | Badge JSONL uses only an in-process lock, has no atomic commit/fsync, skips truncated records, and performs full-file reads. Crash loss, multi-process races, and growth cost remain possible. | Replace with transactional SQLite or an atomic append/index design before enabling multiple workers or high-volume public registry use. | DEFERRED |
...
D-01's marketplace impact is additionally source-proven by `associate_badges()`, which compares only the signed `target_host` to service hosts after dropping non-default ports.

- D-04 is frozen by `tests/test_consent.py::test_audit_requires_no_hard_block_when_consent_missing_and_soft`, which proves that a missing marker still permits the audit when soft consent is configured.
- D-05 is demonstrated by `tests/test_ratelimit.py::test_paid_request_bypasses_rate_limit`, which uses a dummy payment header and receives the elevated bucket.
- D-06 is source-proven by the process-local lock and whole-file reader; `tests/test_security_jsonl_recovery.py` proves corrupt lines are skipped rather than repaired.
